...
const nameServiceHost = process.env.NAME_SERVICE_HOST || 'http://nodejs-istio-security-name:8080';
...
app.get('/api/greeting', (request, response) => {
  nameService(`${nameServiceHost}/api/name`).then(name => {
    ...
    response.status(500);
    response.send(`Failed to communicate with 'nodejs-istio-security-name' due to: ${err}`);
  });
});
